...
async function createPayment(req: Request) {
  ...
  if (userError || !user || user.is_anonymous) {
    return json({ error: "Authenticated email user required" }, 401);
  }
...
async function verifyPayment(url: URL) {
  ...
  const { data: payment, error: paymentError } = await admin
    .from("payments")
    .select("id,user_id,amount,authority,status,subscription_expires_at")
    .eq("id", paymentId)
    .single();
  ...
  if (payment.authority && payment.authority !== authority) {
    return html("پرداخت نامعتبر", "شناسه تراکنش با اطلاعات ثبت‌شده مطابقت ندارد.");
  }
  ...
  // Extend from the later of now or the user's current paid subscription.
  // The pending payment row does not itself contain the previous subscription expiry.
  ...
  const currentExpiry = activeRows?.[0]?.subscription_expires_at
    ? new Date(activeRows[0].subscription_expires_at)
    : new Date(0);
  ...
  if (paidUpdateError) {
    return html("خطای ثبت پرداخت", paidUpdateError.message);
  }
