...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget build(BuildContext context) => Directionality(
        ...
        child: Scaffold(body: SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Icon(Icons.storefront, size: 70), const SizedBox(height: 12),
          const Text('آگهینو', style: TextStyle(fontSize: 34, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
          const Text('خرید و فروش آسان و مطمئن'), const SizedBox(height: 35),
...
class _HomePageState extends State<HomePage> {
  ...
  void openDetails(Map<String, dynamic> ad, List<String> images) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => AdDetailsPage(
      ad: ad, images: images, liked: favorites.contains(id),
      onLike: (value) => setState(() { if (value) { favorites.add(id); } else { favorites.remove(id); } }),
    )));
  }
    ...
    return FutureBuilder<List<String>>(
      ...
      builder: (context, snap) {
        ...
        return Card(clipBehavior: Clip.antiAlias, child: InkWell(
          ...
          child: Row(children: [
            SizedBox(width: 110, height: 110, child: images.isEmpty ? const ColoredBox(color: Color(0xFFEDEDED), child: Icon(Icons.image_outlined, size: 40)) : Image.network(images.first, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image_outlined, size: 40))),
            Expanded(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 7), Text('${ad['price'] ?? 'توافقی'} تومان'), const SizedBox(height: 4), Text('${ad['city'] ?? ''}', style: Theme.of(context).textTheme.bodySmall),
            ]))),
  ...
  Widget home() {
    ...
    return RefreshIndicator(onRefresh: loadAds, child: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 100), children: [
      ...
      const SizedBox(height: 18), const Text('دسته‌بندی‌ها', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)), const SizedBox(height: 10),
      ...
      const SizedBox(height: 22), const Text('جدیدترین آگهی‌ها', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
  ...
  Widget favoritesView() {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 100), children: [const Text('علاقه‌مندی‌ها', style: TextStyle(fontSize: 21, fontWeight: FontWeight.bold)), const SizedBox(height: 10), for (final ad in list) adCard(ad)]);
  }
  Widget account() => ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 100), children: [
    const CircleAvatar(radius: 38, child: Icon(Icons.person, size: 42)), const SizedBox(height: 10), const Center(child: Text('کاربر آگهینو', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))), const SizedBox(height: 18),
    ...
    OutlinedButton.icon(onPressed: () async { await supabase.auth.signOut(); if (!mounted) return; Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const LoginPage()), (_) => false); }, icon: const Icon(Icons.logout), label: const Text('خروج')),
  ]);
  @override
...
class _AdDetailsPageState extends State<AdDetailsPage> {
  ...
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.only(bottom: 30), children: [
      if (widget.images.isEmpty) const SizedBox(height: 240, child: Center(child: Icon(Icons.image_outlined, size: 90))) else SizedBox(height: 280, child: PageView.builder(itemCount: widget.images.length, itemBuilder: (_, i) => Image.network(widget.images[i], fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.broken_image, size: 60))))),
      Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('${widget.ad['title'] ?? 'بدون عنوان'}', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), const SizedBox(height: 12),
        Text('${widget.ad['price'] ?? 'توافقی'} تومان', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w600)), const SizedBox(height: 8),
        Text('${widget.ad['city'] ?? ''} • ${widget.ad['category'] ?? ''}'), const Divider(height: 30),
        const Text('توضیحات', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
        Text('${widget.ad['edescription'] ?? 'توضیحی ثبت نشده است.'}', style: const TextStyle(fontSize: 16, height: 1.7)), const SizedBox(height: 24),
...
class _AddAdPageState extends State<AddAdPage> {
  final title = TextEditingController(); final desc = TextEditingController(); final price = TextEditingController();
  final picker = ImagePicker(); final images = <XFile>[];
  String category = 'کالای دیجیتال'; String city = 'تهران'; bool publishing = false;
  ...
  Future<void> uploadImages(String adId, String userId) async {
    ...
    try {
      for (var i = 0; i < images.length; i++) {
        final im = images[i]; final bytes = await im.readAsBytes(); final name = im.name.toLowerCase();
  ...
  Future<void> publish() async {
    ...
    final user = supabase.auth.currentUser;
    if (user == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ابتدا وارد حساب شوید.'))); return; }
    ...
    try {
      ...
      await cleanup(id); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطای آپلود عکس: ${e.message}')));
    } on PostgrestException catch (e) {
      await cleanup(id); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطای Supabase: ${e.message}')));
    } catch (e) {
      await cleanup(id); if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطا در ثبت آگهی: $e')));
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('ثبت آگهی جدید')), body: ListView(padding: const EdgeInsets.all(16), children: [
      DropdownButtonFormField<String>(value: category, decoration: const InputDecoration(labelText: 'دسته‌بندی', border: OutlineInputBorder()), items: cats.map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: publishing ? null : (v) { if (v != null) setState(() => category = v); }),
      const SizedBox(height: 12), field(title, 'عنوان آگهی'), const SizedBox(height: 12), field(desc, 'توضیحات', lines: 5), const SizedBox(height: 12), field(price, 'قیمت'), const SizedBox(height: 12),
      DropdownButtonFormField<String>(value: city, decoration: const InputDecoration(labelText: 'شهر', border: OutlineInputBorder()), items: cities.map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: publishing ? null : (v) { if (v != null) setState(() => city = v); }),
      const SizedBox(height: 14), OutlinedButton.icon(onPressed: publishing || images.length >= 10 ? null : pickImages, icon: const Icon(Icons.add_a_photo_outlined), label: Text('افزودن عکس (${images.length}/۱۰)')),
      if (images.isNotEmpty) SizedBox(height: 105, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: images.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (context, i) => FutureBuilder<Uint8List>(future: images[i].readAsBytes(), builder: (context, snap) { if (!snap.hasData) return const SizedBox(width: 105, child: Center(child: CircularProgressIndicator())); return Stack(children: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(snap.data!, width: 105, height: 105, fit: BoxFit.cover)), Positioned(top: 0, right: 0, child: IconButton(onPressed: publishing ? null : () => setState(() => images.removeAt(i)), icon: const Icon(Icons.cancel))) ]); })),
      const SizedBox(height: 18), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: publishing ? null : publish, icon: publishing ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.publish), label: Text(publishing ? 'در حال ثبت...' : 'ثبت آگهی'))),
    ])));
  }
}
